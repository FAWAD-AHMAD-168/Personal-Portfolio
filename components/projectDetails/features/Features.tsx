    ...
    >
      ...
      <div className="w-fit grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-x-7 gap-y-9 mt-10">
        {project.features.map((feature, index) => (
          <Card
            key={index}
            className="group w-38 sm:w-52 md:w-69 lg:w-72 xl:w-79 lg:h-60 bg-(--bg-secondary-color) border border-(--primary-color) gap-3 py-4 px-2 md:px-4 hover:-translate-y-2 hover:shadow-[0_0_25px_rgba(156,163,175,0.25)] transition-all duration-300 ease-in-out cursor-pointer"
          >
            ...
            <CardTitle className="text-center text-(--text-primary-color) text-[14px] sm:text-[16px] lg:text-[17px]  ">
              {feature.title}
            </CardTitle>

            <CardDescription className="text-center text-(--text-secondary-color) font-medium text-[12px] sm:text-[14px] lg:text-[13px]">
              {feature.description}
            </CardDescription>
          </Card>
        ))}
